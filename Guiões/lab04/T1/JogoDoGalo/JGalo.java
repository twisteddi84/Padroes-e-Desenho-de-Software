...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
        ...
        if(args.length > 1) {
            System.out.println("Usage: java JGalo [initial_player]");
            System.exit(1);
        } else if(args.length == 1) {
            if (args[0].length() != 1 || (args[0].charAt(0) != 'X' && args[0].charAt(0) != 'O')) {
                System.out.println("Usage: java JGalo [initial_player]");
                System.exit(1);
            } else{
                initial_player = args[0].charAt(0);
            } 
        }
        JogoDoGalo jogo = new JogoDoGalo(initial_player);
        new JGalo(jogo);
	}
}
